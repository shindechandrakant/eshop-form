...
export const VoucherOptionsForm: React.FC<VoucherOptionsFormProps> = ({
  voucherOptions,
  onChange,
}) => {
  ...
  const updateVoucherOption = (
    index: number,
    field: keyof VoucherOption,
    value: string | number
  ) => {
    const updatedOptions = voucherOptions.map((option, i) => {
      if (i === index) {
        ...
        if (field === "discountPercentage" || field === "denomination") {
          const percentage =
            field === "discountPercentage" ? Number(value) : option.discountPercentage || 0;
        ...
        if (field === "discountValue") {
          ...
          const percentage =
            option.denomination > 0 ? Math.round((discountValue / option.denomination) * 100) : 0;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  updateVoucherOption(index, "denomination", parseInt(e.target.value) || 0)
                }
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-2">
                Old Denomination (Optional)
              </label>
              <input
                type="number"
                value={option.oldDenomination || ""}
                onChange={(e) =>
                  updateVoucherOption(
                    index,
                    "oldDenomination",
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  updateVoucherOption(index, "customerPrice", parseInt(e.target.value) || 0)
                }
                className="block w-full px-4 py-3 bg-white border-2 border-slate-200 rounded-xl shadow-sm focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition-all duration-200 text-slate-900 placeholder-slate-400"
                placeholder="475"
              />
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  updateVoucherOption(index, "discountPercentage", parseInt(e.target.value) || 0)
                }
                className="block w-full px-4 py-3 bg-white border-2 border-slate-200 rounded-xl shadow-sm focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition-all duration-200 text-slate-900 placeholder-slate-400"
                placeholder="5"
                min="0"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  updateVoucherOption(index, "discountValue", parseInt(e.target.value) || 0)
                }
                className="block w-full px-4 py-3 bg-white border-2 border-slate-200 rounded-xl shadow-sm focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition-all duration-200 text-slate-900 placeholder-slate-400"
                placeholder="25"
              />
