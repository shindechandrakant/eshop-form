...
import { StoreLocation } from "../types/Cashback";

interface StoreLocatorFormCashbackProps {
  stores: StoreLocation[];
  onChange: (stores: StoreLocation[]) => void;
}

export const StoreLocatorFormCashback: React.FC<StoreLocatorFormCashbackProps> = ({
  stores,
  onChange,
}) => {
  const addStore = () => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="Apple Store New York"
                required
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="block w-full px-4 py-3 bg-white border-2 border-slate-200 rounded-xl shadow-sm focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition-all duration-200 text-slate-900 placeholder-slate-400"
                placeholder="767 Fifth Avenue, New York, NY 10153"
                required
              />
            </div>
          </div>
